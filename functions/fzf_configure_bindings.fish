...
function fzf_configure_bindings --description "Installs the default key bindings for fzf.fish with user overrides passed as options."
    ...
    set -f options_spec h/help 'directory=?' 'git_log=?' 'git_status=?' 'history=?' 'atuin=?' 'processes=?' 'variables=?'
    ...
    if test $status -ne 0
        ...
        # index 1 = directory, 2 = git_log, 3 = git_status, 4 = history, 5=atuin 6 = processes, 7 = variables
        set -f key_sequences \e\cf \e\cl \e\cs \cr \e\cp \cv # \c = control, \e = escape
        set --query _flag_directory && set key_sequences[1] "$_flag_directory"
        set --query _flag_git_log && set key_sequences[2] "$_flag_git_log"
        set --query _flag_git_status && set key_sequences[3] "$_flag_git_status"
        set --query _flag_history && set key_sequences[4] "$_flag_history"
        set --query _flag_atuin && set key_sequences[5] "$_flag_atuin"
        ...
        for mode in default insert
            ...
            test -n $key_sequences[5] && bind --mode $mode $key_sequences[5] _fzf_search_atuin
